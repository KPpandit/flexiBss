...
import { Add, Delete, Save, Schedule } from "@mui/icons-material"

export default function ScheduleDailyMIS() {
  const [enabled, setEnabled] = useState(true)
  const [scheduleTime, setScheduleTime] = useState("09:00")
  const [frequency, setFrequency] = useState("daily")
  const [recipients, setRecipients] = useState(["admin@company.com", "mis@company.com"])
  ...
  const handleSave = () => {
    setSnackbar({
      open: true,
      message: "Daily MIS schedule configuration saved successfully!",
      severity: "success",
    })
  }
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
        ...
        <Typography variant="h4" sx={{ fontWeight: 600 }}>
          Schedule Daily MIS Report
        </Typography>
      </Box>

      <Grid container spacing={3}>
        <Grid item sx={{width:'48%'}} xs={12} md={6}>
          <Card sx={{ height: "100%" }}>
            <CardContent>
        ...
        <Grid item sx={{width:'50%'}} xs={12} md={6}>
          <Card sx={{ height: "100%" }}>
            <CardContent>
              <Typography variant="h6" sx={{ mb: 3, fontWeight: 600 }}>
                Recipients
